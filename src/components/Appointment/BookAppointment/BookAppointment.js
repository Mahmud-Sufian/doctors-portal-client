...
const BookAppointment = ({date}) => {
    return (
        <section className="container">
            <h1 className="text-center" style={{color:'#32A2E0'}}>Available Appointments on {date.toDateString()}</h1>

            <div className="row my-5">
                {
                    bookingData.map(booking => <BookingCard date={date} key={booking._id} booking={booking}></BookingCard>)
                }
            </div>
        </section>
    );
